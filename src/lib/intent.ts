...
const INTENT_PATTERNS: Record<Intent, RegExp[]> = {
  ...
  both_days:      [/^(оба|два|2)\s*дн/i, /ночевк/i],
  only_ceremony:  [/только.*церем/i, /лишь.*церем/i],
  
  flowers: [
    /* прямые формулировки */
    /\bцвет(ы|ок|очек|очков)?\b/i,
    /\bбукет(ы|ик|иков)?\b/i,
    /\bод[ио]н\s+цветок\b/i,
    
    /* распространённые вопросы */
    /(какие|какой)\s+цвет(ы|ок)\s+принести/i,
    /(нужно|нужен|принести|приносить)\s+.*цвет(ок|ы)/i,
    /(один|по\s*одному)\s+цвет(ок|ы)/i,
  ],
  ...
  dresscode: [
    /(dress.?code|дресс.?код|во.?что|одет[ься]|наряд)/i,
    /\bцвета\b/i,              // ровно слово «цвета» (= colours)
  ],
  schedule: [
  ...
  tips: [
    // «полезные советы», «советы по …», «какие-нибудь советы»
    /\bполезн\w*\s+совет\w*/i,
    /\bсовет(ы|ик)?\b/i,                       // просто «совет(ы)»
    // life-hack / лайфхак
    /\bлайф.?хак\w*/i,
    /\blife.?hack\w*/i,
    // «подскажи что-нибудь», «есть ли рекомендации»
    /подскаж\w*.*(совет|как|что|где)/i,
    /\bрекомендац(ия|ии|ий)\b/i,
    /\btips?\b/i,
  ],
  dietary_choice: [
